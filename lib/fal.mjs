...
export async function generateImage({ prompt, outPath, slug = 'fal-ai/nano-banana-2', log = console.log }) {
  const { out, requestId } = await queueRun(slug, { prompt, aspect_ratio: '9:16' }, { log });
...
export async function imageToVideo({ prompt, imageUrl, seconds, outPath, slug = 'fal-ai/veo3.1/fast/image-to-video', log = console.log }) {
  const reqSeconds = [4, 6, 8].find((d) => d >= seconds) ?? 8;
  const { out, requestId } = await queueRun(slug, {
    prompt, image_url: imageUrl, duration: `${reqSeconds}s`, generate_audio: false, resolution: '1080p',
  }, { log });
